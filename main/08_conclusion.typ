...
- We started off in @ch-game by presenting a new iteration upon coinductive
  automata representations in type theory. This puts a new item in the already
  large bag of constructions based on polynomial functors. But perhaps most
  importantly, we demonstrated that with a small twist, namely splitting these
  polynomials in halves, we can obtain well-behaved game descriptions. Thanks to
  this, finely typed automata prove quite suitable to represent strategies for
  dialog games inside type theory.

- Next, in @ch-subst, we introduced a small proof pearl: scope structures. They
  untie the intrinsically scoped and typed theory of substitution from the
  (too) concrete #nm[De-Bruijn] indices. This brings a bit more flexibility in
  the practical formalization of syntactic objects.
...
surface. Formally proving this correctness theorem turned out to be a long
and narrow track. Along the way we zoomed past many opportunities for more in
depth study. Furthermore, although we already capture a number of languages,
the level of generality of our OGS model could be improved. Indeed, we neither
handle effectful languages (apart from partiality), nor polymorphic type
systems, two features for which OGS models have already been
demonstrated~#mcite(dy: -4em, <Laird07>)#mcite(dy: -2em, <LassenL08>)
#mcite(dy: 1em, <JaberT18>). Let us discuss in more details what we managed to
glimpse from a couple of these windows into the unknown.

*Games and Open Composition* #sym.space.quad Although category theory concepts are
structuring our development behind the scenes, we have made the choice to
minimize their amount. Besides making the manuscript slightly more accessible,
it is a pragmatic implementation choice, as category theory mechanization in
intensional type theory is notoriously a can of worms. However, the #nm[Levy]
and #nm[Staton] games and their strategies presented in @ch-game enjoy a rich
categorical theory, which we have mostly skipped over. As a consequence, we did
not say anything about the properties of OGS as a game, besides that it is
symmetric.
...
Like the linear logic $game.xpar$, this #nm[Conway] sum provides a candidate
for game exponentials, as $A de(=>) B := A^game.opp de(+) B$. This combinator
enjoys a corresponding composition operation, taking a strategy on $A^game.opp de(+) B$
and one on $B^game.opp de(+) C$ to a strategy on $A^game.opp de(+) C$. This
could shed new light on our slightly ad-hoc treatment of OGS game strategy
composition. Our hope is that we will then have the necessary scaffolding to
define an _open_ composition which does not return just a final observation,
but a whole OGS strategy. We conjecture that at this point, by ditching this
cumbersome final scope, we will be able to slightly strengthen the adequacy
theorem. This would obtain us a conclusion stronger than correctness, namely
that OGS model equivalence is closed under substitution.
...
setting. Indexation, however, unlocks even more possibilities by building upon
...
are the ones where OGS semantics shine the most. The natural starting point is
...
However, in my opinion, a sometimes overlooked practical insight contributed by
interaction tree, is that partiality should be built in. Notice that in the
above situation, we do not have any $itree.tauF$ node anymore, only
$itree.retF$ and $itree.visF$. Instead, we have traded it with the requirement
that $M$ is iterative, i.e., intuitively it behaves as $delay.t$. Partiality is
ubiquitous when manipulating coinductive game strategies, as for example it is
completely expected that under some circumstances, composition of two total strategies
may fail to be total. We thus conjecture that it would be more fruitful to keep
partiality, and $itree.tauF$ nodes, under our concrete control. We should then
study the following generalization of interaction trees, where $M$ is an
...
Do check the online repository#margin-note(mark: true)[#link("https://github.com/lapin0t/ogs")],
who knows, maybe by the time you are reading these lines my compulsive tendency
to shuffle code around will have made everything tidier! On a more general note,
do not hesitate to reach out to me if any of the above ideas spark your curiosity.
